...
// 紀錄原始request被replace次數
struct replace_list{

	struct list_head next;

	int request;  //紀錄原始 request

	int count;     //紀錄被replace次數

};

static struct replace_list *replace_list_header;

// 紀錄原始request被access次數
struct access_list{
	...
	int count;     //紀錄被access次數

};

static struct access_list *access_list_header;
...
static void add_access_request(unsigned int request);
static void add_replace_request(unsigned int request);
static void delete_all_access_list(void);
static void delete_all_replace_list(void);
...
static long test_ioctl(struct file *fp, unsigned int cmd, unsigned long virtual_addr)
{
	...
	switch(cmd_choice){
		case CMD_WRITE_REQUEST:
			...
			add_access_request(virtual_addr);
...
// 用於紀錄每個request access 次數

static void add_access_request(unsigned int request)

{

	struct access_list *tmp_node;

	int in_access = 0;

	if(list_empty(&access_list_header->next))

	{
		...
		list_for_each_entry(tmp_node, &access_list_header->next, next)

		{

			if(tmp_node->request == request)
...

	{
		...
		tmp_node = kmalloc(sizeof(struct access_list), GFP_KERNEL);

		INIT_LIST_HEAD(&tmp_node->next);

		tmp_node->request = request;

		tmp_node->count = 1;

		

		list_add_tail(&tmp_node->next, &access_list_header->next);
...
// 用於紀錄每個request replace 次數

static void add_replace_request(unsigned int request)

{

	struct replace_list *tmp_node;

	int in_replace = 0;

	if(list_empty(&replace_list_header->next))

	{
		...
		list_for_each_entry(tmp_node, &replace_list_header->next, next)
...

	{
		...
		tmp_node = kmalloc(sizeof(struct replace_list), GFP_KERNEL);
		...
		list_add_tail(&tmp_node->next, &replace_list_header->next);
...
//       		}
    	...
    	add_replace_request(victim_virtual);
        ...
        for(i=currReqNo; i>=0; i--){
	   		...
	   		if(reqMeta[i].virtual_addr == victim_virtual){
				...
				//Modify here
				tmp_node->free_bit = 0;
				tmp_node->virtual_addr = reqMeta[i].virtual_addr;
				tmp_node->ref_count +=1;
...

{
	...
	struct access_list *access_node;

	struct replace_list *replace_node;

	int i = 0;
	int access_max = 0;
	int access_max_request;

	int access_min = 9999;
	int access_min_request;
	printk("==== request access times ====\n");

	if( !list_empty(&access_list_header->next))

	{

		list_for_each_entry(access_node, &access_list_header->next, next){
			
			if(access_node -> count > access_max){
				access_max = access_node -> count;
				access_max_request = access_node -> request;
			}

			if(access_node -> count < access_min){
				access_min = access_node -> count;
				access_min_request = access_node ->request;
			}

			//printk("access request_max:%d , count:%d\n",access_max,access_max_request);
			//printk("access request_min:%d , count:%d\n",access_min,access_min_request);
	...
	printk("==== request replace times ====\n");
	int replace_max = 0;
	int replace_max_request;

	int replace_min = 9999;
	int replace_min_request;
	if( !list_empty(&replace_list_header->next))

	{

		list_for_each_entry(replace_node, &replace_list_header->next, next){
			if(replace_node -> count > replace_max){
				replace_max = replace_node -> count;
				replace_max_request = replace_node -> request;
			}

			if(replace_node -> count < replace_min){
				replace_min = replace_node -> count;
				replace_min_request = replace_node ->request;
			}
			//printk("request request_max:%d , count:%d\n",replace_max,replace_max_request);
			//printk("request request_min:%d , count:%d\n",replace_min,replace_min_request);
...

	{
			...
			printk("virtual_addr:%d, physical_addr:%d, PageRef:%d, free:%d\n", tmp_node->virtual_addr,i,tmp_node->ref_count,tmp_node->free_bit);
	...
	printk("most accessed  :  accessed time : %d, most accessed request  : %d\n", access_max, access_max_request);
	printk("most replaced  :  replaced time : %d, most replaced request  : %d\n", replace_max, replace_max_request);
	printk("least accessed :  accessed time : %d, least accessed request : %d\n", access_min, access_min_request);
	printk("least replaced :  replaced time : %d, least replaced request : %d\n", replace_min, replace_min_request);
